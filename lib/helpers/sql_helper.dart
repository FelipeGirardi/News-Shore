...
class SQLHelper {
  static Future<sql.Database> fetchBookmarksDB() async {
    ...
    return sql.openDatabase(path.join(dbPath, 'bookmarked_news.db'),
        onCreate: (db, version) {
      return db.execute(
          'CREATE TABLE bookmarked_news(id TEXT PRIMARY KEY, title TEXT, link TEXT, keywords TEXT, creator TEXT, video_url TEXT, description TEXT, content TEXT, pub_date TEXT, full_description TEXT, image_url TEXT, source_id TEXT, show_image INTEGER)');
    }, version: 1);
  }
